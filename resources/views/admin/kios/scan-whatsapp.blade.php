...
<div class="mb-6">
    <div class="flex justify-between items-start">
        <div>
            <h2 class="text-xl font-semibold text-gray-800">Scan WhatsApp - {{ $kiosk->name }}</h2>
            <p class="mt-1 text-sm text-gray-600">Hubungkan WhatsApp kios untuk notifikasi otomatis</p>
        </div>
        <x-button variant="secondary" 
            onclick="window.location.href='{{ route('admin.kios.show', $kiosk) }}'">
            Kembali ke Detail
        </x-button>
    </div>
</div>
...
<script>
    ...
    generateQR.addEventListener('click', async () => {
        try {
            const response = await fetch('{{ route("admin.kios.generate-qr", $kiosk) }}');
